(function () {
  ...
  fb.events.fields.証明者所在地_在職.changed.push(function (state) {
    console.log("証明者所在地_在職");
    const newAddress = KanaConverter.halfToFull(
      state.record.証明者所在地_在職.value
    );
    state.record.証明者所在地_在職.value = newAddress;
    console.log(newAddress);
  });

  fb.events.fields.在職証明書に記載する事項_在職.changed.push(function (state) {
    console.log("在職証明書に記載する事項_在職");
    const fullData = KanaConverter.halfToFull(
      state.record.在職証明書に記載する事項_在職.value
    );
    state.record.在職証明書に記載する事項_在職.value = fullData;
    console.log(fullData);
  });

  fb.events.fields.備考_労働者名簿.changed.push(function (state) {
    console.log("備考_労働者名簿");
    const fullData = KanaConverter.halfToFull(
      state.record.備考_労働者名簿.value
    );
    state.record.備考_労働者名簿.value = fullData;
    console.log(fullData);
  });
  ...
  fb.events.fields.退職または死亡事由_労働者名簿.changed.push(function (state) {
    ...
    const fullData = KanaConverter.halfToFull(
      state.record.退職または死亡事由_労働者名簿.value
    );
    state.record.退職または死亡事由_労働者名簿.value = fullData;
    console.log(fullData);
  });

  fb.events.fields.辞令の内容_辞令.changed.push(function (state) {
    console.log("辞令の内容_辞令");
    const fullData = KanaConverter.halfToFull(
      state.record.辞令の内容_辞令.value
    );
    state.record.辞令の内容_辞令.value = fullData;
    console.log(fullData);
  });
  ...
  fb.events.fields.具体的な理由_退職証明.changed.push(function (state) {
    ...
    const fullData = KanaConverter.halfToFull(
      state.record.具体的な理由_退職証明.value
    );
